...
class ProfileController extends Controller
{
    ...
    // personal id image
    public function changePersonalId(Request $request){
        app()->setlocale($request->lang);
        $user=$request->user();
        if($request->image){

            if ($user->getAttributes()['personal_id']){
                $old_image=$user->getAttributes()['personal_id'];
            }
            ...
            $user->save();
            return response()->json($user->personal_id,200);
        }else{
            return $this->error(__('text.Image Required'),404);
        }
    }
    //driving license image
    public function changeDrivingLicense(Request $request){
        app()->setlocale($request->lang);
        $user=$request->user();
        if($request->image){

            if ($user->getAttributes()['driving_license']){
                $old_image=$user->getAttributes()['driving_license'];
            }
            $file=$this->AddSingleImage($request,storage_path('app\public\users\\'));
            $user->update([
            ...
            $user->save();
            return response()->json($user->driving_license,200);
        }else{
            return $this->error(__('text.Image Required'),404);
        }
    }
